...
fun <T, V : Comparable<V>> ValidationBuilder<T, V>.greaterOrEqual(value: V) =
    validate(GreaterOrEqual(value)) { v, _ -> v >= value }

/** A constraint that validate if the value is greater or equal than another property within the same object. */
fun <T, V : Comparable<V>> ValidationBuilder<T, out V>.greaterOrEqual(property: KProperty1<T, V?>) =
